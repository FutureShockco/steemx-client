<script>
import Layout from "@/layouts";
import PageHeader from "@/components/page-header.vue";
import CardHeader from "@/components/card-header.vue";
import RightBar from "@/components/right-bar.vue";

export default {
  data() {
    return {
      dismissibleAlert: false
    };
  },
  components: {
    Layout,
    PageHeader,
    RightBar,
    CardHeader
  },
};
</script>

<template>
  <Layout>
    <div class="page-title-box">
      <BRow class="align-items-center">
        <BCol md="5">
          <PageHeader title="Alerts" pagetitle="Bootstrap UI" />
        </BCol>
        <BCol md="auto" class="ms-auto">
          <RightBar />
        </BCol>
      </BRow>
    </div>
    <BRow>
      <BCol lg="12">
        <BCard no-body>
           <CardHeader title="Default Alerts" content="Use the <code>alert </code> class to show a default alert." />

          <BCardBody class="tab-content">

            <div class="tab-pane show active" id="defaultAlertsPreview" role="tabpanel"
              aria-labelledby="defaultAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary">
                    <strong> Hi! </strong> A simple <b>Primary alert</b> —check it out!
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary">
                    <strong> How are you! </strong> A simple <b>secondary alert</b> —check it out!
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success">
                    <strong> Yey! Everything worked! </strong> A simple <b>success alert</b> —check it out!
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="mb-0">
                    <strong> Something is very wrong! </strong> A simple <b>danger alert</b> —check it out!
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning">
                    <strong> Uh oh, something went wrong </strong> A simple <b>warning alert</b> —check it out!
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info">
                    <strong>Don't forget' it !</strong> A simple <b>info alert</b> —check it out!
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light">
                    <strong>Mind Your Step!</strong> A simple <b>light alert</b> —check it out!
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="mb-0">
                    <strong>Did you know?</strong> A simple <b>dark alert</b> —check it out!
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="defaultAlertsCode" role="tabpanel" aria-labelledby="defaultAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="height: 310px">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Hi! &lt;/strong&gt; A simple &lt;b&gt;Primary alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; How are you! &lt;/strong&gt; A simple &lt;b&gt;secondary alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- success Alert --&gt;
&lt;div class=&quot;alert alert-success&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Yey! Everything worked! &lt;/strong&gt; A simple &lt;b&gt;success alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- danger Alert --&gt;
&lt;div class=&quot;alert alert-danger&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Something is very wrong! &lt;/strong&gt; A simple &lt;b&gt;danger alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- warning Alert --&gt;
&lt;div class=&quot;alert alert-warning&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Uh oh, something went wrong &lt;/strong&gt; A simple &lt;b&gt;warning alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Don't forget' it ! &lt;/strong&gt; A simple &lt;b&gt;info alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Mind Your Step! &lt;/strong&gt; A simple &lt;b&gt;light alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark text-body&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Did you know? &lt;/strong&gt; A simple &lt;b&gt;dark alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Borderless Alerts" content="Use the <code>border-0</code> class to set alert without border." />
          <BCardBody class="tab-content" id="showCodeContent2">
            <div class="tab-pane show active" id="borderlessAlertsPreview" role="tabpanel"
              aria-labelledby="borderlessAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="border-0">
                    <strong> Hi! </strong> A simple <b>Primary alert</b> —check it out!
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="border-0">
                    <strong> How are you! </strong> A simple <b>secondary alert</b> —check it out!
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="border-0">
                    <strong> Yey! Everything worked! </strong> A simple <b>success alert</b> —check it out!
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="border-0 mb-0">
                    <strong> Something is very wrong! </strong> A simple danger alert—check it out!
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="border-0">
                    <strong> Uh oh, something went wrong </strong> A simple <b>warning alert</b> —check it out!
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="border-0">
                    <strong>Don't forget' it !</strong> A simple <b>info alert</b> —check it out!
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="border-0 bg-light">
                    <strong>Mind Your Step!</strong> A simple <b>light alert</b> —check it out!
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="border-0 mb-0">
                    <strong>Did you know?</strong> A simple <b>dark alert</b> —check it out!
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="borderlessAlertsCode" role="tabpanel" aria-labelledby="borderlessAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="height: 305px">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert border-0 alert-primary&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Hi! &lt;/strong&gt; A simple &lt;b&gt;Primary alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert border-0 alert-secondary&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; How are you! &lt;/strong&gt; A simple &lt;b&gt;secondary alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert border-0 alert-success&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Yey! Everything worked! &lt;/strong&gt; A simple &lt;b&gt;success alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert border-0 alert-danger&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Something is very wrong! &lt;/strong&gt; A simple &lt;b&gt;danger alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert border-0 alert-warning&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Uh oh, something went wrong &lt;/strong&gt; A simple &lt;b&gt;warning alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert border-0 alert-info&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Don't forget' it ! &lt;/strong&gt; A simple &lt;b&gt;info alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert border-0 alert-light&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Mind Your Step! &lt;/strong&gt; A simple &lt;b&gt;light alert&lt;/b&gt; &mdash;check it out!  
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert border-0 alert-dark text-body mb-0&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Did you know? &lt;/strong&gt; A simple &lt;b&gt;dark alert&lt;/b&gt; &mdash;check it out!
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Dismissing Alerts" content="Use the <code>alert-dismissible</code> class to add dismissing button to the
            alert." />
          <BCardBody class="tab-content" id="showCodeContent3">
            <div class="tab-pane show active" id="dismissingAlertsPreview" role="tabpanel"
              aria-labelledby="dismissingAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" dismissible>
                    <strong> Hi! </strong> A simple <b>Dismissible primary Alert </b> — check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" dismissible>
                    <strong> How are you! </strong> A simple <b>Dismissible secondary alert</b>
                    —check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" dismissible>
                    <strong>Right Way !</strong> A simple <b>Dismissible success alert</b> —check
                    it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" dismissible class="mb-0">
                    <strong> Something is very wrong! </strong> A simple <b>Dismissible danger alert</b> —check
                    it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" dismissible>
                    <strong>Welcome Back!</strong> A simple <b>Dismissible warning alert</b> —check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" dismissible>
                    <strong>Don't forget' it !</strong> A simple <b>Dismissible info alert</b>
                    —check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" dismissible>
                    <strong>Mind Your Step!</strong> A simple <b>Dismissible light alert</b>
                    —check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" dismissible class="mb-0">
                    <strong>Did you know?</strong> A simple <b>Dismissible dark alert</b>
                    —check it out!
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="dismissingAlertsCode" role="tabpanel" aria-labelledby="dismissingAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="height: 310px">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Hi! &lt;/strong&gt; A simple &lt;b&gt;Dismissible primary Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; How are you! &lt;/strong&gt; A simple &lt;b&gt;Dismissible secondary Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Yey! Everything worked! &lt;/strong&gt; A simple &lt;b&gt;Dismissible success Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Something is very wrong! &lt;/strong&gt; A simple &lt;b&gt;Dismissible danger Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Welcome Back! &lt;/strong&gt; A simple &lt;b&gt;Dismissible warning Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Don't forget' it ! &lt;/strong&gt; A simple &lt;b&gt;Dismissible info Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Mind Your Step! &lt;/strong&gt; A simple &lt;b&gt;Dismissible light Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Did you know? &lt;/strong&gt; A simple &lt;b&gt;Dismissible dark Alert &lt;/b&gt; &mdash; check it out!
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Link Color Alerts" content="Use the <code>alert-link</code> class at &lt;a&gt; tag to show matching colored
            links within the given alert." />
          <BCardBody class="tab-content" id="showCodeContent4">
            <div class="tab-pane show active" id="linkColorAlertsPreview" role="tabpanel"
              aria-labelledby="linkColorAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary">
                    A simple Primary alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary">
                    A simple Secondary alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success">
                    A simple Success alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="mb-0">
                    A simple Danger alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning">
                    A simple Warning alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info">
                    A simple Info alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light">
                    A simple Light alert with <BLink href="#" class="alert-link">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="mb-0">
                    A simple Dark alert with <BLink href="#" class="alert-link text-reset">an example
                      link</BLink>. Give it a click if you like.
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="linkColorAlertsCode" role="tabpanel" aria-labelledby="linkColorAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Primary alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Secondary alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Success alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Danger alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Warning alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Info alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Light alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;</code>

<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark text-body&quot; role=&quot;alert&quot;&gt;
A simple &lt;b&gt;Dark alert&lt;/b&gt; with &lt;a href=&quot;#&quot; class=&quot;alert-link text-reset&quot;&gt;an example link&lt;/a&gt;. Give it a click if you like.
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Live Alert Example" content="Click the Show live alert button to show an alert on button click." />
          <BCardBody class="tab-content" id="showCodeContent5">
            <div class="tab-pane show active" id="liveAlertsPreview" role="tabpanel"
              aria-labelledby="liveAlertsPreview-tab" tabindex="0">
              <div id="liveAlertPlaceholder">
                <BAlert v-model="dismissibleAlert" variant="success" dismissible>
                  Nice, you triggered this alert message!
                </BAlert>
              </div>
              <button type="button" class="btn btn-primary" id="liveAlertBtn" @click="dismissibleAlert = true">Show live
                alert</button>
            </div>
            <div class="tab-pane" id="liveAlertsCode" role="tabpanel" aria-labelledby="liveAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 260px;">
<code>&lt;div id=&quot;liveAlertPlaceholder&quot;&gt;&lt;/div&gt;
&lt;button type=&quot;button&quot; class=&quot;btn btn-primary&quot; id=&quot;liveAlertBtn&quot;&gt;Show live alert&lt;/button&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Outline Alerts" content="Use the <code>bg-body-secondary</code> class to set an alert with outline." />
          <BCardBody class="tab-content" id="showCodeContent6">
            <div class="tab-pane show active" id="outlineAlertsPreview" role="tabpanel"
              aria-labelledby="outlineAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Outline Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="bg-body-secondary" dismissible>
                    <strong> Hi! </strong> - Outline <b>primary alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Outline Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class=" bg-body-secondary" dismissible>
                    <strong> How are you! </strong> - Outline <b>secondary alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Outline Alert</h6>
                  <BAlert :model-value="true" variant="success" class=" bg-body-secondary" dismissible>
                    <strong> Yey! Everything worked! </strong> - Outline <b>success alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Outline Alert</h6>
                  <BAlert :model-value="true" variant="danger" class=" bg-body-secondary mb-0" dismissible>
                    <strong> Something is very wrong! </strong> - Outline <b>danger alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Outline Alert</h6>
                  <BAlert :model-value="true" variant="warning" class=" bg-body-secondary" dismissible>
                    <strong> Uh oh, something went wrong! </strong> - Outline <b>warning alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Outline Alert</h6>
                  <BAlert :model-value="true" variant="info" class=" bg-body-secondary" dismissible>
                    <strong>Don't forget' it !</strong> - Outline <b>info alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class=" bg-body-secondary" dismissible>
                    <strong>Did you know?</strong> - Outline <b>dark alert</b> example
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="outlineAlertsCode" role="tabpanel" aria-labelledby="outlineAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Hi! &lt;/strong&gt; - Outline &lt;b&gt;primary alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; How are you! &lt;/strong&gt; - Outline &lt;b&gt;secondary alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Yey! Everything worked! &lt;/strong&gt; - Outline &lt;b&gt;success alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Something is very wrong! &lt;/strong&gt; - Outline &lt;b&gt;danger alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Uh oh, something went wrong! &lt;/strong&gt; - Outline &lt;b&gt;warning alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Don't forget' it ! &lt;/strong&gt; - Outline &lt;b&gt;info alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-dismissible bg-body-secondary fade show&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt; Did you know? &lt;/strong&gt; - Outline &lt;b&gt;dark alert&lt;/b&gt; example
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Left Border Alerts" content="Use the <code>alert-border-left </code> class to set an alert with the left
            border & outline." />
          <BCardBody class="tab-content" id="showCodeContent7">
            <div class="tab-pane show active" id="leftBorderAlertsPreview" role="tabpanel"
              aria-labelledby="leftBorderAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="alert-border-left" dismissible>
                    <i class="ri-user-smile-line me-3 align-middle fs-lg"></i><strong>Primary</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="alert-border-left" dismissible>
                    <i class="ri-check-double-line me-3 align-middle fs-lg"></i><strong>Secondary</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="alert-border-left" dismissible>
                    <i class="ri-notification-off-line me-3 align-middle fs-lg"></i><strong>Success</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="alert-border-left mb-0" dismissible>
                    <i class="ri-error-warning-line me-3 align-middle fs-lg"></i><strong>Danger</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="alert-border-left" dismissible>
                    <i class="ri-alert-line me-3 align-middle fs-lg"></i><strong>Warning</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="alert-border-left" dismissible>
                    <i class="ri-airplay-line me-3 align-middle fs-lg"></i><strong>Info</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="alert-border-left" dismissible>
                    <i class="ri-mail-line me-3 align-middle fs-lg"></i><strong>Light</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="alert-border-left mb-0" dismissible>
                    <i class="ri-refresh-line me-3 align-middle fs-lg"></i><strong>Dark</strong>
                    - Left border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="leftBorderAlertsCode" role="tabpanel" aria-labelledby="leftBorderAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-user-smile-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Primary&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-check-double-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Secondary&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-notification-off-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Success&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-error-warning-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Danger&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-alert-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Warning&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-airplay-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Info&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-border-left alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-mail-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Light&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-border-left text-body alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-refresh-line me-3 align-middle&quot;&gt;&lt;/i&gt; &lt;strong&gt;Dark&lt;/strong&gt; - Left border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body> 
          <CardHeader title="Label Icon Alerts" content="Use the <code>alert-label-icon</code> class to set an alert with a label icon." />
          <BCardBody class="tab-content" id="showCodeContent7">
            <div class="tab-pane show active" id="labelIconAlertsPreview" role="tabpanel"
              aria-labelledby="labelIconAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="text-white bg-primary alert-label-icon"
                    dismissible>
                    <i class="ri-user-smile-line label-icon"></i><strong>Primary</strong> -
                    Label icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="text-white bg-secondary alert-label-icon"
                    dismissible>
                    <i class="ri-check-double-line label-icon"></i><strong>Secondary</strong>
                    - Label icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="text-white bg-success alert-label-icon"
                    dismissible>
                    <i class="ri-notification-off-line label-icon"></i><strong>Success</strong>
                    - Label icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="text-white bg-danger alert-label-icon mb-0"
                    dismissible>
                    <i class="ri-error-warning-line label-icon"></i><strong>Danger</strong>
                    - Label
                    icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="text-white bg-warning alert-label-icon"
                    dismissible>
                    <i class="ri-alert-line label-icon"></i><strong>Warning</strong> - Label
                    icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="text-white bg-info alert-label-icon" dismissible>
                    <i class="ri-airplay-line label-icon"></i><strong>Info</strong> - Label
                    icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="text-dark bg-light alert-label-icon" dismissible>
                    <i class="ri-mail-line label-icon"></i><strong>Light</strong> -
                    Label icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="text-white bg-dark alert-label-icon mb-0" dismissible>
                    <i class="ri-refresh-line label-icon"></i><strong>Dark</strong> -
                    Label icon alert
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="alert"
                      aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="labelIconAlertsCode" role="tabpanel" aria-labelledby="labelIconAlertsCode-tab"
              tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible text-white bg-primary alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-user-smile-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Primary&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-dismissible text-white bg-secondary alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-check-double-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Secondary&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible text-white bg-success alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-notification-off-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Success&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible text-white bg-danger alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-error-warning-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Danger&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible text-white bg-warning alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-alert-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Warning&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-dismissible text-white bg-info alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-airplay-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Info&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-dismissible text-body bg-light alert-label-icon fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-mail-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Light&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-dismissible text-white bg-dark alert-label-icon fade show mb-0&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-refresh-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Dark&lt;/strong&gt; - Label icon alert
&lt;button type=&quot;button&quot; class=&quot;btn-close btn-close-white&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Additional Content Alerts" content="Use the <code>alert-additional</code> class and Use the <code>alert-</code> class
            to HTML elements like headings, paragraphs, dividers etc." />
          
          <BCardBody class="tab-content" id="showCodeContent8">
            <div class="tab-pane show active" id="additionalContentAlertsPreview" role="tabpanel"
              aria-labelledby="additionalContentAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="alert-additional" dismissible>
                    <div class="alert-body">
                      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                      <div class="d-flex">
                        <div class="flex-shrink-0 me-3">
                          <i class="ri-error-warning-line fs-lg align-middle"></i>
                        </div>
                        <div class="flex-grow-1">
                          <h5 class="alert-heading">Well done !</h5>
                          <p class="mb-0">Aww yeah, you successfully read this important alert message. </p>
                        </div>
                      </div>
                    </div>
                    <div class="alert-content">
                      <p class="mb-0">Whenever you need to, be sure to use margin utilities to keep things nice and tidy.
                      </p>
                    </div>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="alert-additional mb-0" dismissible>
                    <div class="alert-body">
                      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                      <div class="d-flex">
                        <div class="flex-shrink-0 me-3">
                          <i class="ri-alert-line fs-lg align-middle"></i>
                        </div>
                        <div class="flex-grow-1">
                          <h5 class="alert-heading">Something is very wrong!</h5>
                          <p class="mb-0">Change a few things up and try submitting again.</p>
                        </div>
                      </div>
                    </div>
                    <div class="alert-content">
                      <p class="mb-0">Whenever you need to, be sure to use margin
                        utilities to keep things nice and tidy.</p>
                    </div>
                  </BAlert>
                </BCol>

                <BCol xl="6">

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="alert-additional" dismissible>
                    <div class="alert-body">
                      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                      <div class="d-flex">
                        <div class="flex-shrink-0 me-3">
                          <i class="ri-notification-off-line fs-lg align-middle"></i>
                        </div>
                        <div class="flex-grow-1">
                          <h5 class="alert-heading">Yey! Everything worked!</h5>
                          <p class="mb-0">This alert needs your attention, but it's not super important.</p>
                        </div>
                      </div>
                    </div>
                    <div class="alert-content">
                      <p class="mb-0">Whenever you need to, be sure to use margin utilities to keep things nice and tidy.
                      </p>
                    </div>
                  </BAlert>

                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="alert-additional mb-0" dismissible>
                    <div class="alert-body">
                      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                      <div class="d-flex">
                        <div class="flex-shrink-0 me-3">
                          <i class="ri-alert-line fs-lg align-middle"></i>
                        </div>
                        <div class="flex-grow-1">
                          <h5 class="alert-heading">Uh oh, something went wrong!</h5>
                          <p class="mb-0">Better check yourself, you're not looking too good.</p>
                        </div>
                      </div>
                    </div>
                    <div class="alert-content">
                      <p class="mb-0">Whenever you need to, be sure to use margin utilities to keep things nice and tidy.
                      </p>
                    </div>
                  </BAlert>

                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="additionalContentAlertsCode" role="tabpanel"
              aria-labelledby="additionalContentAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible alert-additional fade show&quot; role=&quot;alert&quot;&gt;
&lt;div class=&quot;alert-body&quot;&gt;
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;div class=&quot;d-flex&quot;&gt;
&lt;div class=&quot;flex-shrink-0 me-3&quot;&gt;
&lt;i class=&quot;ri-user-smile-line fs-lg align-middle&quot;&gt;&lt;/i&gt;
&lt;/div&gt;
&lt;div class=&quot;flex-grow-1&quot;&gt;
&lt;h5 class=&quot;alert-heading&quot;&gt;Well done !&lt;/h5&gt;
&lt;p class=&quot;mb-0&quot;&gt;Aww yeah, you successfully read this important alert message. &lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;alert-content&quot;&gt;
&lt;p class=&quot;mb-0&quot;&gt;Whenever you need to, be sure to use margin utilities to keep things nice and tidy.&lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible alert-additional fade show&quot; role=&quot;alert&quot;&gt;
&lt;div class=&quot;alert-body&quot;&gt;
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;div class=&quot;d-flex&quot;&gt;
&lt;div class=&quot;flex-shrink-0 me-3&quot;&gt;
&lt;i class=&quot;ri-error-warning-line fs-lg align-middle&quot;&gt;&lt;/i&gt;
&lt;/div&gt;
&lt;div class=&quot;flex-grow-1&quot;&gt;
&lt;h5 class=&quot;alert-heading&quot;&gt;Something is very wrong!&lt;/h5&gt;
&lt;p class=&quot;mb-0&quot;&gt;Change a few things up and try submitting again. &lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;alert-content&quot;&gt;
&lt;p class=&quot;mb-0&quot;&gt;Whenever you need to, be sure to use margin utilities to keep things nice and tidy.&lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible alert-additional fade show&quot; role=&quot;alert&quot;&gt;
&lt;div class=&quot;alert-body&quot;&gt;
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;div class=&quot;d-flex&quot;&gt;
&lt;div class=&quot;flex-shrink-0 me-3&quot;&gt;
&lt;i class=&quot;ri-notification-off-line fs-lg align-middle&quot;&gt;&lt;/i&gt;
&lt;/div&gt;
&lt;div class=&quot;flex-grow-1&quot;&gt;
&lt;h5 class=&quot;alert-heading&quot;&gt;Yey! Everything worked!&lt;/h5&gt;
&lt;p class=&quot;mb-0&quot;&gt;This alert needs your attention, but it's not super important. &lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;alert-content&quot;&gt;
&lt;p class=&quot;mb-0&quot;&gt;Whenever you need to, be sure to use margin utilities to keep things nice and tidy.&lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;</code>

<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible alert-additional fade show mb-0&quot; role=&quot;alert&quot;&gt;
&lt;div class=&quot;alert-body&quot;&gt;
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;div class=&quot;d-flex&quot;&gt;
&lt;div class=&quot;flex-shrink-0 me-3&quot;&gt;
&lt;i class=&quot;ri-alert-line fs-lg align-middle&quot;&gt;&lt;/i&gt;
&lt;/div&gt;
&lt;div class=&quot;flex-grow-1&quot;&gt;
&lt;h5 class=&quot;alert-heading&quot;&gt;Uh oh, something went wrong!&lt;/h5&gt;
&lt;p class=&quot;mb-0&quot;&gt;Better check yourself, you're not looking too good. &lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;alert-content&quot;&gt;
&lt;p class=&quot;mb-0&quot;&gt;Whenever you need to, be sure to use margin utilities to keep things nice and tidy.&lt;/p&gt;
&lt;/div&gt;
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Top Border with Outline Alerts" content="Use the <code>alert-top-border</code> class to set an alert with the top border
            and outline." />
         
          <BCardBody class="tab-content" id="showCodeContent9">
            <div class="tab-pane show active" id="topBorderWithOutlineAlertsPreview" role="tabpanel"
              aria-labelledby="topBorderWithOutlineAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="alert-top-border" dismissible>
                    <i class="ri-user-smile-line me-3 align-middle fs-lg text-primary"></i><strong>Primary</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>


                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="alert-top-border" dismissible>
                    <i class="ri-check-double-line me-3 align-middle fs-lg text-secondary"></i><strong>Secondary</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="alert-top-border" dismissible>
                    <i class="ri-notification-off-line me-3 align-middle fs-lg text-success"></i><strong>Success</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="alert-top-border mb-0" dismissible>
                    <i class="ri-error-warning-line me-3 align-middle fs-lg text-danger "></i><strong>Danger</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="alert-top-border" dismissible>
                    <i class="ri-alert-line me-3 align-middle fs-lg text-warning"></i><strong>Warning</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="alert-top-border" dismissible>
                    <i class="ri-airplay-line me-3 align-middle fs-lg text-info"></i><strong>Info</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="alert-top-border" dismissible>
                    <i class="ri-mail-line me-3 align-middle fs-lg text-dark"></i><strong>Light</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="alert-top-border mb-0" dismissible>
                    <i class="ri-refresh-line me-3 align-middle fs-lg text-dark"></i><strong>Dark</strong>
                    - Top border alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="topBorderWithOutlineAlertsCode" role="tabpanel"
              aria-labelledby="topBorderWithOutlineAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-user-smile-line me-3 align-middle fs-lg text-primary&quot;&gt;&lt;/i&gt;&lt;strong&gt;Primary&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-check-double-line me-3 align-middle fs-lg text-secondary&quot;&gt;&lt;/i&gt;&lt;strong&gt;Secondary&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-notification-off-line me-3 align-middle fs-lg text-success&quot;&gt;&lt;/i&gt;&lt;strong&gt;Success&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-error-warning-line me-3 align-middle fs-lg text-danger&quot;&gt;&lt;/i&gt;&lt;strong&gt;Danger&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-alert-line me-3 align-middle fs-lg text-warning&quot;&gt;&lt;/i&gt;&lt;strong&gt;Warning&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-airplay-line me-3 align-middle fs-lg text-info&quot;&gt;&lt;/i&gt;&lt;strong&gt;Info&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-top-border alert-dismissible fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-mail-line me-3 align-middle fs-lg text-light&quot;&gt;&lt;/i&gt;&lt;strong&gt;Light&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-top-border alert-dismissible text-body fade show mb-0&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-refresh-line me-3 align-middle fs-lg text-dark&quot;&gt;&lt;/i&gt;&lt;strong&gt;Dark&lt;/strong&gt; - Top border alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Label Icon Arrow Alerts" content="Use the <code>alert-label-icon label-arrow</code> class to show an alert with
            label icon and arrow." />
          <BCardBody class="tab-content" id="showCodeContent10">
            <div class="tab-pane show active" id="labelIconArrowAlertsPreview" role="tabpanel"
              aria-labelledby="labelIconArrowAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-user-smile-line label-icon"></i><strong>Primary</strong> -
                    Label icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-check-double-line label-icon"></i><strong>Secondary</strong> -
                    Label icon
                    arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-notification-off-line label-icon"></i><strong>Success</strong>
                    - Label
                    icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="alert-label-icon label-arrow mb-0" dismissible>
                    <i class="ri-error-warning-line label-icon"></i><strong>Danger</strong>
                    - Label
                    icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-alert-line label-icon"></i><strong>Warning</strong> -
                    Label icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-airplay-line label-icon"></i><strong>Info</strong> -
                    Label icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="alert-label-icon label-arrow" dismissible>
                    <i class="ri-mail-line label-icon"></i><strong>Light</strong>
                    - Label icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="alert-label-icon label-arrow mb-0" dismissible>
                    <i class="ri-refresh-line label-icon"></i><strong>Dark</strong>
                    - Label icon arrow alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="labelIconArrowAlertsCode" role="tabpanel"
              aria-labelledby="labelIconArrowAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-user-smile-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Primary&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-check-double-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Secondary&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-notification-off-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Success&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-error-warning-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Danger&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-alert-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Warning&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-airplay-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Info&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-dismissible alert-label-icon label-arrow fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-mail-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Light&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-dismissible alert-label-icon label-arrow text-body fade show mb-0&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-refresh-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Dark&lt;/strong&gt; - Label icon arrow  alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot; alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <CardHeader title="Rounded Label Icon Alerts" content="Use the <code>alert-label-icon rounded-label</code> class to set an alert with a
            rounded label icon." />
          <BCardBody class="tab-content" id="showCodeContent11">
            <div class="tab-pane show active" id="roundedLabelIconArrowAlertsPreview" role="tabpanel"
              aria-labelledby="roundedLabelIconArrowAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-user-smile-line label-icon"></i><strong>Primary</strong> -
                    Rounded label alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Secondary Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-check-double-line label-icon"></i><strong>Secondary</strong>
                    - Rounded
                    label alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Success Alert</h6>
                  <BAlert :model-value="true" variant="success" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-notification-off-line label-icon"></i><strong>Success</strong>
                    - Rounded
                    label alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Danger Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="alert-label-icon rounded-label mb-0" dismissible>
                    <i class="ri-error-warning-line label-icon"></i><strong>Danger</strong>
                    - Rounded
                    label alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-alert-line label-icon"></i><strong>Warning</strong> - Rounded
                    label alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Info Alert</h6>
                  <BAlert :model-value="true" variant="info" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-airplay-line label-icon"></i><strong>Info</strong> -
                    Rounded label
                    alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Light Alert</h6>
                  <BAlert :model-value="true" variant="light" class="alert-label-icon rounded-label" dismissible>
                    <i class="ri-mail-line label-icon"></i><strong>Light</strong> -
                    Rounded label
                    alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>

                  <h6>Dark Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="alert-label-icon rounded-label mb-0" dismissible>
                    <i class="ri-refresh-line label-icon"></i><strong>Dark</strong> -
                    Rounded label
                    alert
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                  </BAlert>
                </BCol>

              </BRow>
            </div>
            <div class="tab-pane" id="roundedLabelIconArrowAlertsCode" role="tabpanel"
              aria-labelledby="roundedLabelIconArrowAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-user-smile-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Primary&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-check-double-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Secondary&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-notification-off-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Success&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-error-warning-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Danger&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-alert-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Warning&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-airplay-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Info&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light alert-dismissible alert-label-icon rounded-label fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-mail-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Light&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark alert-dismissible alert-label-icon rounded-label text-body fade show&quot; role=&quot;alert&quot;&gt;
&lt;i class=&quot;ri-refresh-line label-icon&quot;&gt;&lt;/i&gt;&lt;strong&gt;Dark&lt;/strong&gt; - Rounded label alert
&lt;button type=&quot;button&quot; class=&quot;btn-close&quot; data-bs-dismiss=&quot;alert&quot; aria-label=&quot;Close&quot;&gt;&lt;/button&gt;
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>

    <BRow>
      <BCol lg="12">
        <BCard no-body>         
          <CardHeader title="Solid Alerts" content="Use the <code>border-*</code>, <code>bg-*</code>, <code>text-white</code> class
            to set an alert with solid style." />

          <BCardBody class="tab-content" id="showCodeContent12">
            <div class="tab-pane show active" id="solidAlertsPreview" role="tabpanel"
              aria-labelledby="solidAlertsPreview-tab" tabindex="0">
              <BRow>
                <BCol xl="6">
                  <h6>Primary Solid Alert</h6>
                  <BAlert :model-value="true" variant="primary" class="border-primary bg-primary text-white">

                    <strong>Hi!</strong> - Solid <b>primary alert</b> example
                  </BAlert>

                  <h6>Secondary Solid Alert</h6>
                  <BAlert :model-value="true" variant="secondary" class="border-secondary bg-secondary text-white">
                    <strong>How are you!</strong> - Solid <b>secondary alert</b> example
                  </BAlert>

                  <h6>Success Solid Alert</h6>
                  <BAlert :model-value="true" variant="success" class="border-success bg-success text-white">
                    <strong>Yey! Everything worked! </strong> - Solid <b>success alert</b> example
                  </BAlert>

                  <h6>Danger Solid Alert</h6>
                  <BAlert :model-value="true" variant="danger" class="border-danger bg-danger text-white mb-0">
                    <strong>Something is very wrong!</strong> - Solid <b>danger alert</b> example
                  </BAlert>
                </BCol>

                <BCol xl="6">
                  <h6>Warning Solid Alert</h6>
                  <BAlert :model-value="true" variant="warning" class="border-warning bg-warning text-white">
                    <strong>Uh oh, something went wrong!</strong> - Solid <b>warning alert</b> example
                  </BAlert>

                  <h6>Info Solid Alert</h6>
                  <BAlert :model-value="true" variant="info" class="border-info bg-info text-white">
                    <strong>Don't forget' it !</strong> - Solid <b>info alert</b> example
                  </BAlert>


                  <h6>Light Solid Alert</h6>
                  <BAlert :model-value="true" variant="light" class="border-light bg-light text-dark">
                    <strong>Mind Your Step!</strong> - Solid <b>secondary alert</b> example
                  </BAlert>

                  <h6>Dark Solid Alert</h6>
                  <BAlert :model-value="true" variant="dark" class="border-dark bg-dark text-white mb-0">
                    <strong>Did you know?</strong> - Solid <b>dark alert</b> example
                  </BAlert>

                </BCol>
              </BRow>
            </div>
            <div class="tab-pane" id="solidAlertsCode" role="tabpanel" aria-labelledby="solidAlertsCode-tab" tabindex="0">
              <h5 class="fs-sm text-muted">HTML Preview</h5>
              <pre class="language-markup" style="max-height: 310px;">
<code>&lt;!-- Primary Alert --&gt;
&lt;div class=&quot;alert alert-primary border-primary bg-primary text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Hi!&lt;/strong&gt; - Solid &lt;b&gt;primary alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Secondary Alert --&gt;
&lt;div class=&quot;alert alert-secondary border-secondary bg-secondary text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;How are you!&lt;/strong&gt; - Solid &lt;b&gt;secondary alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Success Alert --&gt;
&lt;div class=&quot;alert alert-success border-success bg-success text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Yey! Everything worked!&lt;/strong&gt; - Solid &lt;b&gt;success alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Danger Alert --&gt;
&lt;div class=&quot;alert alert-danger border-danger bg-danger text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Something is very wrong!&lt;/strong&gt; - Solid &lt;b&gt;danger alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Warning Alert --&gt;
&lt;div class=&quot;alert alert-warning border-warning bg-warning text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Uh oh, something went w rong!&lt;/strong&gt; - Solid &lt;b&gt;warning alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Info Alert --&gt;
&lt;div class=&quot;alert alert-info border-info bg-info text-white&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Don't forget' it !&lt;/strong&gt; - Solid &lt;b&gt;info alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Light Alert --&gt;
&lt;div class=&quot;alert alert-light border-light bg-light text-body&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Mind Your Step!&lt;/strong&gt; - Solid &lt;b&gt;light alert&lt;/b&gt; example
&lt;/div&gt;
</code>
<code>&lt;!-- Dark Alert --&gt;
&lt;div class=&quot;alert alert-dark border-dark bg-dark text-white mb-0&quot; role=&quot;alert&quot;&gt;
&lt;strong&gt;Did you know?&lt;/strong&gt; - Solid &lt;b&gt;dark alert&lt;/b&gt; example
&lt;/div&gt;</code></pre>
            </div>
          </BCardBody>
        </BCard>
      </BCol>
    </BRow>
  </Layout>
</template>